...
@section('title', 'Comunidades Lingüísticas')
@section('breadcrumb', 'Comunidades Lingüísticas')
            ...
            <div class="card mb-4">
                <div class="card-header pb-0 bg-brand-header">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h6 class="text-white mb-0">Comunidades Lingüísticas</h6>
                            <p class="text-sm text-white opacity-8 mb-0">
                                Este módulo permite gestionar las comunidades lingüísticas registradas en el sistema.
                            </p>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="{{ route('linguistic-communities.create') }}" class="btn btn-sm btn-white">
                                <i class="bi bi-plus me-2"></i>Nueva Comunidad Lingüística
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body pt-3 pb-2">
                    <form action="{{ url('/linguistic-communities') }}" method="GET" class="mb-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-auto ms-2">
                                <a href="{{ url('/linguistic-communities?status=' . $status) }}" class="btn btn-outline-secondary">
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0 dataTable-table" id="datatable-basic" data-datatable="true">
                            ...
                            <tbody>
                                @forelse($linguisticCommunities as $community)
                                <tr>
                                    <td>
                                        <div class="d-flex px-3 py-2">
                                            <div class="avatar avatar-sm me-3 bg-brand-header rounded-circle">
                                                <span class="text-white font-weight-bold">{{ substr($community->name, 0, 1) }}</span>
                                            </div>
                                    ...
                                    <td class="px-3 py-2" style="width: 35%; min-width: 300px; max-width: 500px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                        <p class="text-sm text-secondary mb-0 d-flex align-items-center" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;">
                                            ...
                                            <span class="ms-2">
                                                <i class="bi bi-info-circle text-info" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $community->description }}"></i>
                                            </span>
                                        </p>
                                    </td>
                                    <td class="align-middle text-center">
                                        @if ($status === 'active')
                                            <a href="{{ route('linguistic-communities.edit', $community) }}" class="btn bg-brand-header rounded-pill px-3 py-2 me-2">
                                                <i class="bi bi-pencil me-1"></i>Editar
                                            </a>
                                            <form action="{{ route('linguistic-communities.destroy', $community) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @else
                                            <form action="{{ route('linguistic-communities.reactivate', $community->id) }}" method="POST" class="d-inline">
                                                @csrf
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-4">
                                        <span class="text-muted">No hay comunidades lingüísticas registradas.</span>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
