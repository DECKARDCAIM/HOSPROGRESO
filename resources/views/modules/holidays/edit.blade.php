...
@section('breadcrumb', 'Días Festivos / Editar')
                ...
                <div class="card mb-4">
                    <div class="card-header pb-0 bg-brand-header">
                        <div class="row align-items-center">
                            <div class="col-md-8">
                                <h6 class="text-white mb-0">Editar Día Festivo</h6>
                                <p class="text-sm text-white opacity-8 mb-0">
                                    Modificar información del día festivo: {{ $holiday->name }}
                                </p>
                            </div>
                            <div class="col-md-4 text-end">
                                <a href="{{ route('holidays.index', ['status' => 'active', 'year' => $holiday->date->year]) }}" class="btn btn-sm btn-white">
                        ...
                        <form method="POST" action="{{ route('holidays.update', $holiday) }}">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" 
                                          ...
                                          name="description" 
                                          rows="3" 
                                          placeholder="Descripción opcional del día festivo">{{ old('description', $holiday->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="d-flex justify-content-end mt-4">
                                <button type="button" class="btn btn-secondary btn-lg me-2" onclick="window.location.href='{{ route('holidays.index', ['status' => 'active', 'year' => $holiday->date->year]) }}'">
                                    <i class="bi bi-x me-2"></i>Cancelar
                                </button>
                                <button type="submit" class="btn bg-brand-header btn-lg text-white">
                                    <i class="bi bi-check-lg me-2"></i>Actualizar día festivo
                                </button>
                            </div>
                        </form>
                    </div>
