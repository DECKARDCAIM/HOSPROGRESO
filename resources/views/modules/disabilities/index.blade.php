...
@section('breadcrumb', 'Discapacidades')
            ...
            <div class="card mb-4">
                <div class="card-header pb-0 bg-brand-header">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h6 class="text-white mb-0">Discapacidades</h6>
                            <p class="text-sm text-white opacity-8 mb-0">
                                Este módulo permite gestionar las discapacidades registradas en el sistema.
                            </p>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="{{ route('disabilities.create') }}" class="btn btn-sm btn-white">
                                <i class="bi bi-plus me-2"></i>Nueva Discapacidad
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body pt-3 pb-2">
                    <form action="{{ url('/disabilities') }}" method="GET" class="mb-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-auto ms-2">
                                <a href="{{ url('/disabilities?status=' . $status) }}" class="btn btn-outline-secondary">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex px-3 py-2">
                                            <div class="avatar avatar-sm me-3 bg-brand-header rounded-circle">
                                                <span class="text-white font-weight-bold">{{ substr($disability->name, 0, 1) }}</span>
                                            </div>
                                            <div class="d-flex flex-column justify-content-center">
                                                <h6 class="mb-0 text-sm">{{ $disability->name }}</h6>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-3 py-2">
                                        @if($disability->is_active)
                                    ...
                                    <td class="px-3 py-2" style="width: 35%; min-width: 300px; max-width: 500px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                        <p class="text-sm text-secondary mb-0 d-flex align-items-center" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;">
                                            {{ Str::limit($disability->description, 60) }}
                                            <span class="ms-2">
                                                <i class="bi bi-info-circle text-info" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $disability->description }}"></i>
                                            </span>
                                        </p>
                                    </td>
                                    <td class="align-middle text-center">
                                        @if ($status === 'active')
                                            <a href="{{ route('disabilities.edit', $disability) }}" class="btn bg-brand-header rounded-pill px-3 py-2 me-2">
                                        ...
                                        @else
                                            <form action="{{ route('disabilities.reactivate', $disability->id) }}" method="POST" class="d-inline">
                                                @csrf
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-4">
                                        <span class="text-muted">No hay discapacidades registradas.</span>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
