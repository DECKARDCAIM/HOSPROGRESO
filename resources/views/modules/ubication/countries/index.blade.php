            ...
            <div class="card mb-4">
                <div class="card-header pb-0 bg-info">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h6 class="text-white mb-0">Países</h6>
                            <p class="text-sm text-white opacity-8 mb-0">
                                Este módulo permite gestionar los países.
                            </p>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="{{ url('/paises/create') }}" class="btn btn-sm btn-white">
                                <i class="fas fa-plus me-2"></i>Agregar País
                            </a>
                        </div>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="d-flex px-3 py-2">
                                                <div class="avatar avatar-sm me-3 bg-info rounded-circle">
                                                    <span class="text-white font-weight-bold">{{ substr($pais->name, 0, 1) }}</span>
                                                </div>
                                        ...
                                        <td class="px-3 py-2" style="width: 35%; min-width: 300px; max-width: 500px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                            <p class="text-sm text-secondary mb-0 d-flex align-items-center" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;">
                                                {{ Str::limit($pais->description, 60) }}
                                                <span class="ms-2">
                                                    <i class="fas fa-info-circle text-info" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $pais->description }}"></i>
                                                </span>
                                            </p>
                                        </td>
                                        <td class="align-middle text-center">
                                            @if ($status === 'active')
                                                <a href="{{ url('/paises/' . $pais->id . '/edit') }}" class="btn btn-info rounded-pill px-3 py-2 me-2">
                                                    <i class="fas fa-edit me-1"></i> Editar
                                                </a>
                                                <form action="{{ url('/paises/' . $pais->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @else
                                                <form action="{{ route('paises.reactivate', $pais->id) }}" method="POST" class="d-inline">
                                                    @csrf
