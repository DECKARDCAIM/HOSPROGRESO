                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-4 text-end">
                                <a href="{{ url('/municipios/create') }}" class="btn btn-sm btn-white">
                                    <i class="bi bi-plus me-2"></i>Nuevo Municipio
                                </a>
                            </div>
                        </div>
                    </div>
                        ...
                        <form action="{{ url('/municipios') }}" method="GET" class="mb-0">
                            <div class="row align-items-center">
                                <div class="col-md-2 mb-2 mb-md-0">
                                    <select name="status"
                                        class="form-select form-select-lg border border-info auto-submit">
                                ...
                                <tbody>
                                    @forelse($municipalities as $municipality)
                                        <tr>
                                            <td>
                                                <div class="d-flex px-3 py-2">
                                                    <div class="avatar avatar-sm me-3 bg-brand-header rounded-circle">
                                                        <span
                                                            class="text-white font-weight-bold">{{ substr($municipality->name, 0, 1) }}</span>
                                                    </div>
                                            ...
                                            <td class="px-3 py-2">
                                                <span
                                                    class="text-sm text-secondary">{{ $municipality->department->name ?? 'Sin departamento' }}</span>
                                            </td>
                                            <td class="px-3 py-2">
                                                ...
                                                <p class="text-sm text-secondary mb-0 d-flex align-items-center"
                                                    ...
                                                    <span class="ms-2">
                                                        <i class="bi bi-info-circle text-info" data-bs-toggle="tooltip"
                                                            ...
                                                            title="{{ $municipality->description }}"></i>
                                                    </span>
                                                </p>
                                            </td>
                                            <td class="align-middle text-center">
                                                ...
                                                @else
                                                    <form
                                                        action="{{ url('/municipios/' . $municipality->id . '/reactivate') }}"
                                                        method="POST" class="d-inline">
                                                        @csrf
                                                        <button type="submit"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (countrySelect && departmentSelect) {
                    ...
                    window.allDepartments.forEach(dept => {
                        if (dept.country_id == countryId) {
                            departmentSelect.innerHTML +=
                                `<option value="${dept.id}"${selectedId == dept.id ? ' selected' : ''}>${dept.name}</option>`;
                            hasDepartments = true;
                        }
                    });
                ...
                if (countrySelect.value) {
                    filterDepartmentsByCountry(countrySelect.value, '{{ request("department_id") }}');
                } else {
                    departmentSelect.innerHTML = '<option value="">Todos los departamentos</option>';
                }
            }
