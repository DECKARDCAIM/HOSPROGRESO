...
@section('breadcrumb', 'Estados Civiles')
            ...
            <div class="card mb-4">
                <div class="card-header pb-0 bg-brand-header">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h6 class="text-white mb-0">Estados Civiles</h6>
                            <p class="text-sm text-white opacity-8 mb-0">
                                Este módulo permite gestionar los estados civiles registrados en el sistema.
                            </p>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="{{ route('civil-statuses.create') }}" class="btn btn-sm btn-white">
                                <i class="bi bi-plus me-2"></i>Nuevo Estado Civil
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body pt-3 pb-2">
                    <form action="{{ url('/civil-statuses') }}" method="GET" class="mb-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-auto ms-2">
                                <a href="{{ url('/civil-statuses?status=' . $status) }}" class="btn btn-outline-secondary">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex px-3 py-2">
                                            <div class="avatar avatar-sm me-3 bg-brand-header rounded-circle">
                                                <span class="text-white font-weight-bold">{{ substr($status->name, 0, 1) }}</span>
                                            </div>
                                    ...
                                    <td class="px-3 py-2" style="width: 35%; min-width: 300px; max-width: 500px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                        <p class="text-sm text-secondary mb-0 d-flex align-items-center" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;">
                                            {{ Str::limit($status->description, 60) }}
                                            <span class="ms-2">
                                                <i class="bi bi-info-circle text-info" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $status->description }}"></i>
                                            </span>
                                        </p>
                                    </td>
                                    <td class="align-middle text-center">
                                        @if ($status->is_active)
                                            <a href="{{ route('civil-statuses.edit', $status) }}" class="btn bg-brand-header rounded-pill px-3 py-2 me-2">
                                                <i class="bi bi-pencil me-1"></i>Editar
                                            </a>
                                            <form action="{{ route('civil-statuses.destroy', $status) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @else
                                            <form action="{{ route('civil-statuses.reactivate', $status->id) }}" method="POST" class="d-inline">
                                                @csrf
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-4">
                                        <span class="text-muted">No hay estados civiles registrados.</span>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
