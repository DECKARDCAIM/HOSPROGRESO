...
@section('breadcrumb', 'Pruebas de Laboratorio')
            ...
            <div class="card mb-4">
                <div class="card-header pb-0 bg-brand-header">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <h6 class="text-white mb-0">Pruebas de Laboratorio</h6>
                            <p class="text-sm text-white opacity-8 mb-0">
                                Este módulo permite gestionar las pruebas de laboratorio registradas en el sistema.
                            </p>
                        </div>
                        <div class="col-md-4 text-end">
                            <a href="{{ route('laboratory-tests.create') }}" class="btn btn-sm btn-white">
                                <i class="bi bi-plus me-2"></i>Nueva Prueba
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body pt-3 pb-2">
                    <form action="{{ url('/laboratory-tests') }}" method="GET" class="mb-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-auto ms-2">
                                <a href="{{ url('/laboratory-tests?status=' . $status) }}" class="btn btn-outline-secondary">
                ...
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0 dataTable-table" id="datatable-basic" data-datatable="true">
                            ...
                            <tbody>
                                @forelse($laboratoryTests as $test)
                                <tr>
                                    <td>
                                        <div class="d-flex px-3 py-2">
                                            <div class="avatar avatar-sm me-3 bg-brand-header rounded-circle">
                                                <span class="text-white font-weight-bold">{{ substr($test->name, 0, 1) }}</span>
                                            </div>
                                    ...
                                    <td class="px-3 py-2" style="width: 35%; min-width: 300px; max-width: 500px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                        <p class="text-sm text-secondary mb-0 d-flex align-items-center" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;">
                                            ...
                                            <span class="ms-2">
                                                <i class="bi bi-info-circle text-info" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ $test->description }}"></i>
                                            </span>
                                        </p>
                                    </td>
                                    <td class="align-middle text-center">
                                        @if ($status === 'active')
                                            <a href="{{ route('laboratory-tests.edit', $test) }}" class="btn bg-brand-header rounded-pill px-3 py-2 me-2">
                                                <i class="bi bi-pencil me-1"></i>Editar
                                            </a>
                                            <form action="{{ route('laboratory-tests.destroy', $test) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @else
                                            <form action="{{ route('laboratory-tests.reactivate', $test->id) }}" method="POST" class="d-inline">
                                                @csrf
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-4">
                                        <span class="text-muted">No hay pruebas de laboratorio registradas.</span>
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
